...
def submit(request, course_id):
    ...
    for submit in submitted_anwsers:
        choice = Choice.objects.get(pk=submit)
...
def show_exam_result(request, course_id, submission_id):
    ...
    choices = list(submission.choices.all())
    choice_ids = []
    questions_list = []
    for choice in choices:
        choice_ids.append(choice.id)
        questions_list.append(choice.question)

    grade = 0
    unique = list(set(questions_list))
    ...
    for question in questions_list:
        print(question.is_get_score(choice_ids))
        flags.append(question.is_get_score(choice_ids))
    entries = {}
    for key in choices:
        for value in flags:
            entries[key] = value
